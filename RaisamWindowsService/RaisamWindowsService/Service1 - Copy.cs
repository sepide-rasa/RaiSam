...
namespace RaisamWindowsService
{
    public partial class Service1 : ServiceBase
    {
        ...
        protected override void OnStart(string[] args)
        {
            ...
            timScheduledTask.Interval = 2 * 60 * 1000;    
        ...
        void timScheduledTask_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            //System.IO.File.AppendAllText(AppDomain.CurrentDomain.BaseDirectory + @"\Uploaded\a.txt", "vorood   "+ "\n");
                ...
                //var g = m.prs_GetDate().FirstOrDefault();
                    ...
                    conn = new SqlConnection("Data Source=192.168.1.34;Initial Catalog=RaiSam;User ID=rasasystem;Password=rasasystem!@#");
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("dbo.prs_GetDate", conn);
                    ...
                    using (SqlDataReader rdr = cmd.ExecuteReader())
                    {
                        ...
                        while (rdr.Read())
                        {
                             DateRooz = rdr["fldDateTime"].ToString();
                             TarikhRooz = rdr["fldTarikh"].ToString();
                        }
                    }

                 sw = new StreamWriter(AppDomain.CurrentDomain.BaseDirectory + "\\a.txt", true);
                 sw.WriteLine("raft to: " + DateRooz);
                sw.Flush();
                sw.Close();

              //  System.IO.File.AppendAllText(AppDomain.CurrentDomain.BaseDirectory + @"\Uploaded\a.txt", "raft to: " + g.fldDateTime + "\n");
                getSajamData(TarikhRooz);
                    //Select
                    //var sms=m.prs_tblSafSMSSelect("Pass3Day", "", 0).ToList();
                SqlCommand cmd2 = new SqlCommand("dbo.prs_tblSafSMSSelect", conn);
                    cmd2.CommandType = CommandType.StoredProcedure;
                    cmd2.Parameters.Add(new SqlParameter("@fieldname", "Pass3Day"));
                    cmd2.Parameters.Add(new SqlParameter("@value", ""));
                    cmd2.Parameters.Add(new SqlParameter("@h", "0"));
                    ...
                    //{
                    //ersal payam
                            CallWebServiceSms(rdr2["fldMatn"].ToString(), rdr2["fldMobile"].ToString());
                    //Update
                       // m.prs_tblSafSMSUpdate(2, item.fldAshkhasId, item.fldCherkheFirstEghdamId, 1);
                            SqlCommand cmd3 = new SqlCommand("dbo.prs_tblSafSMSUpdate", conn);
                        cmd3.CommandType = CommandType.StoredProcedure;
                        cmd3.Parameters.Add(new SqlParameter("@fldStatus", "1"));
                        cmd3.Parameters.Add(new SqlParameter("@fldAshkhasId", rdr2["fldAshkhasId"].ToString()));
                        cmd3.Parameters.Add(new SqlParameter("@fldCherkheFirstEghdamId", rdr2["fldCherkheFirstEghdamId"].ToString()));
                        cmd3.Parameters.Add(new SqlParameter("@fldInputId", "1"));
                        cmd3.ExecuteReader();
                        //insert mojadad hamun record
                       // m.prs_tblSafSMSInsert(item.fldMatn,1,item.fldAshkhasId,item.fldCherkheFirstEghdamId, 1);
                        SqlCommand cmd4 = new SqlCommand("dbo.prs_tblSafSMSInsert", conn);
                        cmd4.CommandType = CommandType.StoredProcedure;
                        cmd4.Parameters.Add(new SqlParameter("@fldMatn", rdr2["fldMatn"].ToString()));
                        cmd4.Parameters.Add(new SqlParameter("@fldStatus", "1"));
                        cmd4.Parameters.Add(new SqlParameter("@fldAshkhasId", rdr2["fldAshkhasId"].ToString()));
                        cmd4.Parameters.Add(new SqlParameter("@fldCherkheFirstEghdamId", rdr2["fldCherkheFirstEghdamId"].ToString()));
                        cmd4.Parameters.Add(new SqlParameter("@fldInputId", "1"));
                        cmd4.ExecuteReader();
        ...
        void getSajamData(string TarikhRooz)
        {
            SqlConnection conn = null;
            conn = new SqlConnection("Data Source=192.168.1.34;Initial Catalog=RaiSam;User ID=rasasystem;Password=rasasystem!@#");
            ...
            //    SejamService.SejamService sv = new SejamService.SejamService();
                ...
                try
                {
                  ...
                  //  var MoveTran = sv.GetMoveTranDetails(g.fldTarikh, g.fldTarikh, null, null);
                    var MoveTran = GetMoveTranDetails(TarikhRooz, TarikhRooz, null, null);
                    ...
                    if (MoveTran.Err == 0)
                    {
                        foreach (var item in MoveTran.data)
                        {
                            ...
                            if (/*item.PelakNo != 0 &&*/ item.SalonNo != 0)
                            {
                                ...
                                SqlCommand cmd3 = new SqlCommand("dbo.prs_InsertFromWebServiceSajam", conn);
                                cmd3.CommandType = CommandType.StoredProcedure;

                                cmd3.Parameters.Add(new SqlParameter("@TrainNum", item.TranNo.ToString()));
                                cmd3.Parameters.Add(new SqlParameter("@MoveDateTrain", item.MoveDate.ToString()));
                                cmd3.Parameters.Add(new SqlParameter("@MoveTimeTrain", item.MoveTime.ToString()));
                                cmd3.Parameters.Add(new SqlParameter("@PelakNo", item.PelakNo.ToString()));
                                cmd3.Parameters.Add(new SqlParameter("@SalonNo", item.SalonNo.ToString()));
                                cmd3.Parameters.Add(new SqlParameter("@StartSation", item.SourceStation.ToString()));
                                cmd3.Parameters.Add(new SqlParameter("@EndStation", item.TargetStation.ToString()));
                                cmd3.ExecuteReader();
                            }
                        }
                    ...
                    {
                       // m.prs_tblErrorInsert(ErrorId, MoveTran.Msg, DateTime.Now, 1, "تاریخ: " + TarikhRooz + "(SajamWinService1)");
                        SqlCommand cmd2 = new SqlCommand("dbo.prs_tblErrorInsert", conn);
                        cmd2.CommandType = CommandType.StoredProcedure;

                        cmd2.Parameters.Add(new SqlParameter("@fldID", ErrorId.ToString()));
                        cmd2.Parameters.Add(new SqlParameter("@fldMatn", MoveTran.Msg.ToString()));
                        cmd2.Parameters.Add(new SqlParameter("@fldTarikh", DateTime.Now.ToString()));
                        cmd2.Parameters.Add(new SqlParameter("@fldInputID", "1"));
                        cmd2.Parameters.Add(new SqlParameter("@fldDesc", "تاریخ: " + TarikhRooz + "(SajamWinService1)"));
                        cmd2.ExecuteReader();
                    }
                    conn.Close();
                }
                catch (Exception x)
                {
                    ...
                    if (x.InnerException != null)
                        ...
                        InnerException = x.Message;
                    //m.prs_tblErrorInsert(ErrorId1, InnerException, DateTime.Now, 1, "تاریخ: " + TarikhRooz + "(SajamWinService2)");
                    SqlCommand cmd2 = new SqlCommand("dbo.prs_tblErrorInsert", conn);
                    cmd2.CommandType = CommandType.StoredProcedure;

                    cmd2.Parameters.Add(new SqlParameter("@fldID", ErrorId1.ToString()));
                    cmd2.Parameters.Add(new SqlParameter("@fldMatn", InnerException));
                    cmd2.Parameters.Add(new SqlParameter("@fldTarikh", DateTime.Now.ToString()));
                    cmd2.Parameters.Add(new SqlParameter("@fldInputID", "1"));
                    cmd2.Parameters.Add(new SqlParameter("@fldDesc", "تاریخ: " + TarikhRooz + "(SajamWinService2)"));
                    cmd2.ExecuteReader();
